...
class GaussianNoiseSNR(AudioTransformPerChannel):
    ...
    def apply(self, y: np.ndarray):
        ...
        if y.ndim == 1:
            white_noise = np.random.randn(len(y))  # for 1D
        elif y.ndim == 2:
            white_noise = np.random.randn(y.shape[0], y.shape[1])  # for 2D
        #print(y.shape)
        #print(y.shape, white_noise.shape, snr)
...
class GaussianNoiseSNRTorch(AudioTransformPerChannel):
    ...
    def apply(self, y: torch.Tensor):
        snr = np.random.uniform(self.min_snr, self.max_snr)
        if y.ndim == 1:
            white_noise = torch.randn(len(y))  # for 1D
        elif y.ndim == 2:
            white_noise = torch.randn(y.shape[0], y.shape[1])  # for 2D
        elif y.ndim == 3:
            white_noise = torch.randn(y.shape[0], y.shape[1], y.shape[2])  # for ch + 2D
        #print(y.shape)
        #print(y.shape, white_noise.shape, snr)
        return add_noise_snr_torch(y, white_noise.to(y.device), snr)
...
class PinkNoiseSNRTorch(AudioTransformPerChannel):
    '''
    Pink noise: exponent = 1
    Brown noise: exponent = 2
    '''
    ...
    def apply(self, y: torch.Tensor):
        snr = np.random.uniform(self.min_snr, self.max_snr)
        pink_noise = cn.powerlaw_psd_gaussian(self.exponent, len(y))
        pink_noise = torch.tensor(pink_noise)
        #print(y.shape, pink_noise.shape, snr)
        return add_noise_snr_torch(y, pink_noise.to(y.device), snr)
