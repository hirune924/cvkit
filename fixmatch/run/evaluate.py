...
def main(args):
    ...
    valid_df = df[(df['fold']==config.target_fold)&(df['labeled']==1)].reset_index(drop=True)
    unlbl_df = df[df['labeled']==0].reset_index(drop=True)
    valid_df = pd.concat([valid_df, unlbl_df]).reset_index(drop=True)
    ...
    valid_df = result[result['labeled']==1]
    logger.info('metrics:\n' + str(metrics.classification_report(valid_df['class_id'], valid_df['predict'], target_names=config.classes)))
