...
def main(args):
    ...
    unlabeled_index = random.sample(list(range(len(df))), int(len(df)*(1-args.labeled_ratio)))
    df.loc[unlabeled_index,'class_id'] = None
    df.loc[unlabeled_index,'class'] = None
    df.loc[unlabeled_index,'labeled'] = 0

    logger.info('Save')
    df = df[['image_path', 'class_id', 'class', 'labeled', 'fold']]
